...
def create_train_val_dataloader(opt, logger):
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            if 'ConStyle_v2' not in opt['model_type']:
                ...
                logger.info(f'Number of val images/folders in ImageNet: {len(val_set)}')
...
def train_pipeline(root_path):
    ...
    if opt['datasets']['train'].get('progressive_learning'):
        ...
        scale = opt['datasets']['train'].get('scale')
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        while train_data is not None:
            ...
            if current_iter % opt['logger']['print_freq'] == 0:
                ...
                if opt['model_type'] == 'ConStyle_v2':
                    log_vars.update({'top1(%)': model.train_top1.avg, 'top5(%)': model.train_top5.avg})
            ...
            if current_iter % opt['val']['val_freq'] == 0:
                if opt['model_type'] == 'ConStyle' or opt['model_type'] == 'ConStyle_v2_origin':
                    logger.info('Saving ConStyle model and training state.')
                    ...
                    if opt['model_type'] == 'ConStyle_v2':
                        model.save(epoch, current_iter)
                        model.train_top1.reset()
                        model.train_top5.reset()
                        logger.info('Saving ConStyle_v2 model and training state.')
